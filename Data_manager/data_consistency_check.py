...
def assert_URM_ICM_mapper_consistency(URM_DICT, user_original_ID_to_index, item_original_ID_to_index,
    ...
    nonzero_items = np.arange(0, n_items_URM, dtype=np.int32)[nonzero_items_mask]
    ...
    nonzero_users = np.arange(0, n_users_URM, dtype=np.int32)[nonzero_users_mask]
    ...
    if UCM_MAPPER_DICT is not None:
        ...
        for UCM_name, UCM_object in UCM_DICT.items():
            ...
            nonzero_features = np.arange(0, n_features, dtype=np.int32)[nonzero_features_mask]
